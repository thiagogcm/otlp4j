/// The OTLP/gRPC transport: `OtlpGrpcExporter` and `OtlpGrpcReceiver`, built on gRPC + Netty. The
/// only module that pulls in `grpc-netty-shaded`, so an HTTP-only deployment never carries Netty.
