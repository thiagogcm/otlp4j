...
/// `/v1development/profiles`) and routes decoded requests to the per-signal dispatchers passed in by
/// [HttpOtlpServerProvider]. [Tls] selects the server: [Tls.Disabled] is plaintext and [Tls.Custom]
/// supplies the certificate and key (via an [HttpsServer]); [Tls.SystemTrust] is rejected, as a
/// server has no certificate of its own.
///
/// When no [ServerTransportConfig#serverExecutor()] is configured the server uses a
/// virtual-thread-per-request executor, so a handler blocking on its dispatcher's [CompletionStage]
/// never starves the others.
final class HttpOtlpServer implements OtlpServer {
    ...
    HttpOtlpServer(ServerTransportConfig config, OtlpServerProvider.Dispatchers dispatchers) {
        this.config = config;
